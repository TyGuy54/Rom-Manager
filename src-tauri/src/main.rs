...
fn card_data() -> Result<Vec<HashMap<&'static str, Value>>, Error>{
    ...
    let paths = fs::read_dir("").unwrap(); // nned to set path to ROMS/GBA
    ...
    for roms in paths {
        ...
        match rom_file_extension.unwrap() {
            "gba" => { 
                ...
                card_data_payload.insert("rom_name", Value::OsStr(rom_name.into_string().unwrap()));
...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![load_rom, card_data])
